...
invisible to callers. In C#, split a type into `partial` files named
`Type.Concern.cs`. A split that only balances line counts is worse than the
long file; find the real seam.
...
The FFI crate produces a platform-specific native library: `libprosody_ffi.dylib`
(macOS), `libprosody_ffi.so` (Linux), or `prosody_ffi.dll` (Windows). The
Makefile detects the platform and copies the library to where .NET finds it.
...
Integration tests require Kafka and Cassandra. Start them with
...
Run tests with `make test`, or directly with
...
**Integration tests:** Write slow test output to a file and grep the file. Do not pipe it to `grep`,
`head`, or `tail`: that loses the output and forces an expensive re-run.
...
`#[allow(...)]` is **FORBIDDEN** in this project. Never add one. If you find an
existing `#[allow(...)]`, remove it and fix the underlying issue properly.

If you believe an exception is truly necessary, **ask permission first** and
explain why the warning cannot be fixed. If permission is granted, use
`#[expect(...)]` (not `#[allow(...)]`) with a reason:

```rust
#[expect(clippy::some_lint, reason = "explanation approved by maintainer")]
```

Warnings from proc-macro generated code (e.g., UniFFI macros) must still be
addressed. Adjust the source code to avoid the warning, or request an
`#[expect]` exception if the warning is unavoidable.
...
- CSharpier for formatting (`dotnet tool run csharpier format .`)
...
Regenerate bindings after Rust API changes with `make bindgen` (or `make build`
to rebuild everything). The assembly-wide `DefaultDllImportSearchPaths` in
`src/Prosody/AssemblyAttributes.cs` satisfies CA5392 for the generated P/Invoke
declarations.
...
- Kafka (`apache/kafka`) and Cassandra for integration tests
